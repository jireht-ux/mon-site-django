...
def process_invoice(pdf_path):
    ...
    # Image pour LayoutLMv3
    pix = page.get_pixmap()
    img = Image.frombytes("RGB", [pix.width, pix.height], pix.samples)
    
    # Extraction texte avec tri spatial
...
def generate_dcp_report(blocks):
    """Fusionne les libellés/valeurs et génère le rapport d'audit final."""
    if not blocks: return {}

    # 1. Fusion Sémantique Horizontale (ex: Total TTC + Valeur)
    merged = []
    skip = set()
    # On trie par position Y pour faciliter la fusion par ligne
    ...
    for i in range(len(sorted_blocks)):
        ...
        for j in range(i + 1, len(sorted_blocks)):
            ...
            # Si même ligne et éloigné horizontalement (gouttière blanche)
            same_line = abs(curr['top'] - cand['top']) < 0.015
            is_key_value = any(k in curr['content'].upper() for k in ["TOTAL", "TTC", "HT", "N°", "DATE"])
            
            if same_line and is_key_value:
    ...
    # 2. Identification des DCP et catégorisation
    dcp_zones = {}
    patterns = {
    ...
    for i, block in enumerate(merged):
        ...
        # Logique métier : Emetteur vs Client
...
# --- EXECUTION ---
try:
    raw_results = process_invoice("facture_2.pdf")
    final_report = generate_dcp_report(raw_results)
